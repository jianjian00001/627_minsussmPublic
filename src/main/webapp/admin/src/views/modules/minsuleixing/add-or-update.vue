<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="民宿类型" prop="minsuleixing">
          <el-input v-model="ruleForm.minsuleixing" 
              placeholder="民宿类型" clearable  :readonly="ro.minsuleixing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="民宿类型" prop="minsuleixing">
              <el-input v-model="ruleForm.minsuleixing" 
                placeholder="民宿类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='minsuleixing'){
            this.ruleForm.minsuleixing = obj[o];
	    this.ro.minsuleixing = true;
            continue;
          }
        }
      }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `minsuleixing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.minsuleixingCrossAddOrUpdateFlag = false;
						 this.parent.search();
